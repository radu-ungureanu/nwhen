...
public class WhenWeeklyRruleTests
{
    ...
    public void TestWeeklyFour()
    {
        ...
        Assert.Equal(10, r1.Occurrences.Count);
        for (int i = 0; i < results.Length; i++)
            Assert.Equal(Fmt(results[i]), Fmt(r1.Occurrences[i]));
    ...
    public void TestWeeklyFive()
    {
        ...
        Assert.Equal(25, r.Occurrences.Count);
        for (int i = 0; i < results.Length; i++)
            Assert.Equal(Fmt(results[i]), Fmt(r.Occurrences[i]));
    }
    ...
    public void TestWeeklySix()
    {
        ...
        Assert.Equal(8, r.Occurrences.Count);
        for (int i = 0; i < results.Length; i++)
            Assert.Equal(Fmt(results[i]), Fmt(r.Occurrences[i]));
    }
    ...
    /// <summary>Every 1st Mon or Fri each week for 4 weeks via BYSETPOS=1</summary>
    [Fact]
    ...
    /// <summary>Until date before generated time: FREQ=WEEKLY;BYDAY=TU;BYHOUR=18 with until earlier same day</summary>
    [Fact]
    public void TestGeneratedTimeGreaterThanUntilDate()
    {
